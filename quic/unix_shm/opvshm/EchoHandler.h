...
class DlwHandler : public quic::QuicSocket::ConnectionSetupCallback,
 ...
 public:
  ...
  void setShmConfig(int64_t client_id, std::shared_ptr<client_info_t> client_info, int uds_conn_fd){
    client_info_ = client_info;
    uds_conn_fd_ = uds_conn_fd;
    connId_ = client_id;
    ...
    try {
        ...
        shm_fifo_read_handler_->changeHandlerFD(folly::NetworkSocket::fromFd(uds_conn_fd_));
        shm_fifo_read_handler_->registerHandler(folly::EventHandler::READ | folly::EventHandler::PERSIST);
        LOG(INFO) << "Event handler registered for listen FIFO fd: " << uds_conn_fd_;
  ...
  void handleFifoReadEvent() {
      ...
      ssize_t bytes_read = ::read(uds_conn_fd_, recv_length_, sizeof(ssize_t));
      if (bytes_read <= 0) {
      ...
      sentBytes_[connId] += shm_bytes_read;
 ...
 private:
  ...
  void SendDataToApp(const char* data, size_t dataLength) {
    ...
    VLOG(2) << " data="
            << ((data) ? [&](){
                ...
                auto str = std::string(data, dataLength);
  ...
  void handleReqToShm(quic::StreamId id, StreamData& data) {
      ...
      if (pos < httpData.length()) {
        ...
        *notify_buffer_ = send_bytes; // 'R' for Request
        ...
        receivedBytes_[connId_] += requestData.length();
      }
  ...
  void calculateThroughput() {
    while (true) {
      ...
      for(int i = 0; i < 2; i++){
        auto currentByte = receivedBytes_[i] + sentBytes_[i];
        auto throughput = (currentByte - previousBytes_[i]) * 8 / 1024 / 1024; // in MB
        auto receivedThroughput = (receivedBytes_[i] - previousReceivedBytes_[i]) * 8 / 1024 / 1024; // in MB
        auto sentThroughput = (sentBytes_[i] - previousSentBytes_[i]) * 8 / 1024 / 1024; // in MB
        LOG(INFO) << "Client " << i << " Current throughput: " << throughput << " MB/s, "
                  << "received throughput: " << receivedThroughput << " MB/s, "
                  << "sent throughput: " << sentThroughput << " MB/s";
        previousBytes_[i] = currentByte;
        previousReceivedBytes_[i] = receivedBytes_[i];
        previousSentBytes_[i] = sentBytes_[i];
      }
    }
  }
  ...
  std::unordered_map<int64_t, std::streamsize> receivedBytes_;
  std::unordered_map<int64_t, std::streamsize> sentBytes_;
  std::unordered_map<int64_t, std::streamsize> previousReceivedBytes_;
  std::unordered_map<int64_t, std::streamsize> previousSentBytes_;
  std::unordered_map<int64_t, std::streamsize> previousBytes_;
