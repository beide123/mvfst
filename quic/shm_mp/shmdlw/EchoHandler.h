...
class DlwHandler : public quic::QuicSocket::ConnectionSetupCallback,
 ...
 public:
  ...
  void setShmConfig(int64_t client_id, config_t *config, shm_config_t *shm_config, sem_t *sem_r, sem_t *sem_w){
    ...
    memcpy(&client_info->config, config, sizeof(config_t));
    memcpy(&client_info->shm_config, shm_config, sizeof(shm_config_t));
 ...
 private:
  ...
  void shmRecv(quic::StreamId id) {
    ...
    while(1){
        ...
        bytesRead = shm_read(client_info->shm_sock, client_info->recv_buff, sizeof(client_info->recv_buff));
        if(bytesRead < 0){
            perror("shm_read failed");
            break;
        }
        VLOG(5) << "Server read request " << client_info->recv_buff << " from shm.";

        if(bytesRead == 3 && client_info->recv_buff[0] == 'E'){
            if(strncmp(client_info->recv_buff, "EOF", bytesRead) == 0){
        ...
        memcpy(rspbuf + headerSize, client_info->recv_buff, bytesRead);
  ...
  void handleShmRequest(quic::StreamId id, StreamData& data) {
    ...
    while((pos = httpData.find("\r\n\r\n", start)) != std::string::npos){
        ...
        if(requestType == "ACTIVATE"){
            ...
            snprintf(client_info->send_buff, sizeof(client_info->send_buff), "%s", requestData.c_str());

            ssize_t send_bytes = -1;
            send_bytes = shm_write(client_info->shm_sock, client_info->send_buff, requestData.length());
            if (send_bytes < 0) {
                if (send_bytes == -1) {
  ...
  client_info_t *client_info{nullptr};
};
