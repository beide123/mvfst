...
class UDPAcceptor : public folly::AsyncUDPSocket::ReadCallback {
 public:
  ...
  void onDataAvailable(
    ...
    if (!transport_ || !transport_->good()) {
      quicHandler_ = std::make_shared<DlwHandler>(evb_);
 ...
 private:
  ...
  std::shared_ptr<DlwHandler> quicHandler_;
  std::shared_ptr<quic::QuicServerTransport> transport_;
  std::unique_ptr<folly::IOBuf> readBuffer_;
};

class ShmMdlwTransportServer {
 public:
  explicit ShmMdlwTransportServer(std::string host = "::1", uint16_t port = 6666)
